...
const { Engine, Render, World, Bodies, Events, Runner, Vector } = Matter;

// Variable global simple para resolver la caída de la pelota
let globalBallDropResolver = null;
let currentBall = null; 

// Control de cooldown para el sonido de rebote
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const plinkoOpSignSpan = document.getElementById('plinko-op-sign');
    ...
    // Reproducción más eficiente sin clonación infinita
    ...
    function setupPhysicsBoard() {
        ...
        // --- CAMBIO CLAVE: Definimos 6 filas a dibujar ---
        const rowsToDraw = 6; 
        
        slotWidth = boardWidth / NUM_SLOTS;
        const colSpacing = slotWidth;
        
        // Calculamos el espaciado basándonos en el 85% de la altura total
        // para asegurar espacio sobre las ranuras.
        const effectiveHeight = boardHeight * 0.85; 
        const rowSpacing = effectiveHeight / rowsToDraw; 
        // ---------------------------------------------------------

        for (let row = 0; row < rowsToDraw; row++) { 
            // Posición Y basada en el nuevo espaciado
        ...
        Events.on(engine, 'collisionStart', (event) => {
            ...
            for (let i = 0; i < pairs.length; i++) {
                ...
                const PIN_FILL_STYLE = '#3cb371'; 
                // Detectar si una pelota golpea un clavo 
                if (
        ...
        Events.on(engine, 'afterUpdate', () => {
            if (currentBall && globalBallDropResolver) {
                // Si la pelota está cerca de la parte inferior
    ...
    startGameBtn.addEventListener('click', () => {
     ...
     // --- CAMBIO CLAVE AQUÍ: Desbloquear el audio ---
        try {
            // Intentamos reproducir un sonido al momento del clic del usuario
            audioHit.volume = 0.01; // Volumen casi inaudible
            audioHit.play();
            audioHit.volume = 1.0; // Restablecer el volumen normal
        } catch (e) {
            console.warn("Audio no pudo ser desbloqueado inmediatamente.", e);
        }
        // ------------------------------------------------
